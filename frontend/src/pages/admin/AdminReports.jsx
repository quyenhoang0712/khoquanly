...
export default function AdminReports() {
  ...
  return (
    <section className="page">
      ...
      <div className="panel table-wrap"><table><thead><tr><th>Ngày</th><th>Nhân viên</th><th>Công việc</th><th>Nội dung</th><th>Ảnh</th></tr></thead><tbody>
        {rows.length === 0 && <EmptyRow colSpan={5} />}
        {rows.map((row) => <tr key={row._id}><td>{formatDate(row.date)}</td><td>{row.user?.name}</td><td>{row.task?.title}</td><td>{row.content}</td><td><div className="image-list">{row.images?.map((img) => <a key={img} href={`${API_ORIGIN}${img}`} target="_blank"><img src={`${API_ORIGIN}${img}`} alt="report" /></a>)}</div></td></tr>)}
      </tbody></table></div>
    </section>
  );
}
